...
export default function RiwayatTransaksiPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-extrabold text-white font-[Outfit]">Riwayat Transaksi Penjualan</h2>
        <p className="text-xs text-coffee-400">Daftar transaksi penjualan terdaftar di {user?.cabang?.nama_cabang || 'Cabang'}</p>
      </div>
            ...
            <table className="w-full text-sm text-left">
              <thead>
                <tr className="border-b border-white/10 text-coffee-400">
                  ...
                  <th className="pb-3 font-semibold text-right">Total Transaksi</th>
              ...
              <tbody>
                {transactions.map(tx => (
                  <tr key={tx.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="py-3.5 text-white font-semibold">{tx.kasir?.nama || 'Kasir'}</td>
                ...
                {transactions.length === 0 && (
                  <tr>
                    <td colSpan={6} className="py-12 text-center text-coffee-500">
                      ...
                      <p className="text-sm font-semibold">Belum ada transaksi di cabang ini</p>
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="bg-coffee-800 border border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div id="receipt-print" className="text-center space-y-3 font-mono text-xs text-coffee-200">
              ...
              <div className="text-left text-[10px] space-y-1 text-coffee-400">
                <p>Kasir: {selectedTx.kasir?.nama || 'Kasir'}</p>
              ...
              <div className="border-t border-dashed border-white/20 my-2" />
              <p className="text-accent italic text-[10px] font-semibold">Reprint Struk Transaksi ☕</p>
            </div>
